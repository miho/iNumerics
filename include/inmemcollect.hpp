...
namespace iNumerics
{
	...
	******************************************************************************/
	...
	void MemCollect<T>::freeMem ( MemType<T>*& mem )
	{
		...
		if ( mem != NULL )
		{
			...
			if ( !mem->isReferenced() )
			{
				...
				if ( freeMemList.size() > 0 )
				{
					typename std::deque< MemType<T>* >::iterator i = freeMemList.end();
	...
	{
		while ( freeMemList.size() > 0 )
		{
			delMem ( *freeMemList.begin() );
		}
	}
	...
	******************************************************************************/
	...
	void MemCollect<T>::delMem ( MemType<T>*& mem )
	{
		...
		if ( mem != NULL )
		{
			...
			for ( typename std::deque< MemType<T>* >::iterator i = freeMemList.begin(); i != freeMemList.end();i++ )
			{
				if ( *i == mem )
				{
					detachFromMemList ( i );
					...
					// Debug-Output
					IN_DISPLAY ( ">> List-Size: " << freeMemList.size() << " (" << MByte ( _memListSize ).value() << " MByte)",2 );
